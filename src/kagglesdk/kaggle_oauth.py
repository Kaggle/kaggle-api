...
class KaggleOAuth:
  ...
  class OAuthState:
    def __init__(self):
      ...
      print(f"{self.code_verifier} / {self.code_challenge}")
    ...
    def _handle_oauth_callback(self):
      ...
      if "code" in query_params and "state" in query_params:
        ...
        if state == self._oauth_state.state:
          ...
          logger.error(f"Invalid state! Expected: {self._oauth_state.state}, Received: {state}")
  ...
  def _run_oauth_flow(self, scopes: list[str], no_launch_browser: bool) -> KaggleCredentials:
    ...
    if use_browser:
      ...
      logger.debug(f"Will listen the callback at: {redirect_uri}")
    ...
    if use_browser:
      ...
      def handler_factory(*args, **kwargs):
        return KaggleOAuth.OAuthCallbackHandler(
          *args, oauth=self, oauth_state=oauth_state, on_success=exchange_oauth_token, **kwargs)
      ...
      with socketserver.TCPServer(("127.0.0.1", port), handler_factory) as httpd:
        ...
        logger.debug("OAuth flow completed (or server stopped).")
    else:
      print("\nGo to the following link in your browser, and complete the sign-in prompts at Kaggle:\n")
      print(f"  {oauth_start_url}")
      print("\nOnce finished, enter the verification code provided in your browser: ", end="")
      code = input()
      self._exchange_oauth_token(code, scopes, oauth_state)
