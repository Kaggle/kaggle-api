...
class ApiListUserManagedGroupMembershipsResponse(KaggleObject):
  r"""
  Attributes:
    memberships (ApiListUserManagedGroupMembershipsResponse.Membership)
    next_page_token (str)
  """

  class Membership(KaggleObject):
    r"""
    Attributes:
      user_id (int)
      username (str)
  ...
  @property
  def memberships(self) -> Optional[List[Optional['ApiListUserManagedGroupMembershipsResponse.Membership']]]:
    return self._memberships

  @memberships.setter
  def memberships(self, memberships: Optional[List[Optional['ApiListUserManagedGroupMembershipsResponse.Membership']]]):
    if memberships is None:
      del self.memberships
      return
    if not isinstance(memberships, list):
      raise TypeError('memberships must be of type list')
    if not all([isinstance(t, ApiListUserManagedGroupMembershipsResponse.Membership) for t in memberships]):
      raise TypeError('memberships must contain only items of type ApiListUserManagedGroupMembershipsResponse.Membership')
...
ApiListUserManagedGroupMembershipsResponse.Membership._fields = [
...
ApiListUserManagedGroupMembershipsResponse._fields = [
  FieldMetadata("memberships", "memberships", "_memberships", ApiListUserManagedGroupMembershipsResponse.Membership, [], ListSerializer(KaggleObjectSerializer())),
